...
def fit_white_light(
    ...
    t, lcs, lcs_err, orbits, xs, ys, bkg = [], [], [], [], [], [], []
    for v in obs:
        t.append(v.time)
        lcs.append(v.average_lc / v.average_lc.mean())
        lcs_err.append(v.average_lc_errors / v.average_lc.mean())
        orbits.append(v.orbits)
        xs.append(v.xshift / v.xshift.std())
        ys.append(v.yshift / v.yshift.std())
        bkg.append(v.bkg / v.bkg.std())
    ...
    hook2 = X([-np.exp(-300 * (t1 - t1[0])) for t1 in t]) ** 2
    ...
    if supplement is not None:
        ...
        x_suppl, y_suppl, yerr_suppl, exptime_suppl = (
            np.asarray(supplement.time, np.float64),
    ...
    eclipse_lc = obs.sys.flux(x - obs.sys.secondaries[0].porb.eval() / 2).eval()
    if model_lc.sum() == 0:
    ...
    if no_fit:
        A1 = np.hstack([A, eclipse_lc[:, None] - 1, model_lc[:, None] - 1])
        ...
        obs.eclipse_lc = (eclipse_lc[np.argsort(x)] - 1) * w[-2]
        obs.eclipse_depth = -np.min(w[-2] * (eclipse_lc - 2))

        obs.noise_model = A1[:, :-2].dot(w[:-2])
        obs.noise_model *= np.hstack(
            [v.average_lc.mean() * v.average_lc ** 0 for v in obs]
        )
        ...
        for idx, visit in enumerate(obs):
            ...
            visit.eclipse_lc = (eclipse_lc[np.in1d(x, t[idx])] - 1) * w[-2]
        return
    ...
    r = fit_transit(
        ...
        period_val=obs.sys.secondaries[0].porb.eval(),
    ...
    obs.eclipse_lc = map_soln["eclipse"]
    obs.eclipse_depth = (map_soln["eclipse_r"] / map_soln["r_star"]) ** 2

    obs.noise_model = (map_soln["noise_model"]) + map_soln["normalization"]
    obs.noise_model *= np.hstack([v.average_lc.mean() * v.average_lc ** 0 for v in obs])
    ...
    for idx, visit in enumerate(obs):
        ...
        visit.eclipse_lc = obs.eclipse_lc[np.in1d(x, t[idx])]
