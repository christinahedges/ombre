...
def simple_mask(sci, filter="G141"):
    with warnings.catch_warnings():
        ...
        if filter == "G102":
            edge = (
                np.where((np.gradient(spectral / np.nanmax(spectral)) < -0.07))[0][0]
                - 10
            )
...
def average_vsr(obs):
    avg = np.atleast_3d(np.average(obs.data, weights=1 / obs.error, axis=1)).transpose(
        [0, 2, 1]
    )
    vsr_mean = np.asarray(
        [
            np.average(
                obs.data[idx] / avg[idx], weights=avg[idx] / obs.error[idx], axis=1
            )
...
def average_spectrum(obs):
    if hasattr(obs, "vsr_mean"):
        ...
        avg = np.atleast_3d(
            np.atleast_3d(
                np.average(obs.data, weights=1 / obs.error, axis=2)
            ).transpose([0, 2, 1])
        )
    spec_mean = np.average(obs.data / avg, weights=avg / obs.error, axis=(0, 1))
    spec_mean /= np.mean(spec_mean)
    return (
        np.atleast_3d(spec_mean) * np.ones(obs.shape).transpose([0, 2, 1])
    ).transpose([0, 2, 1])
...
def get_flatfield(obs, visit, model):
    ...
    f = f[
        :,
        1014 // 2 - visit.ns // 2 : 1014 // 2 + visit.ns // 2,
        1014 // 2 - visit.ns // 2 : 1014 // 2 + visit.ns // 2,
    ]

    f = np.vstack([f, np.ones((1, f.shape[1], f.shape[2]))])
    X = f[:, visit.spatial.reshape(-1)][:, :, visit.spectral.reshape(-1)][
        :, np.ones((nrow, ncol), bool)
    ].T
    avg = np.nanmean((visit.data / norm / model), axis=0)[np.ones((nrow, ncol), bool)]
    avg_err = (
        (1 / nt) * np.nansum(((visit.error / norm / model)) ** 2, axis=0) ** 0.5
    )[np.ones((nrow, ncol), bool)]
    ...
    X = f[:, np.ones((visit.ns, visit.ns), bool)].T
    flat = np.dot(X, w).reshape((visit.ns, visit.ns))
    flat /= np.median(flat)
    flat[(visit.dq[0] & 256) != 0] = 1
...
def calibrate(obs, teff=5500, plot=False):
    ...
    for count in [0, 1, 2]:
        ...
        def func(params, return_model=False):
            ...
            if return_model:
                return model
            return np.nansum((obs.spec_mean[0, 0] - model) ** 2) / (
        ...
        sens /= np.median(sens)
        sensitivity = np.interp(wavelength, wav.value, sens)
...
def build_vsr(obs, errors=False):
    ...
    frames_err = (
        frames_err.T / np.average(frames, weights=1 / obs.error, axis=(1, 2))
    ).T
    ...
    for tdx in tqdm(range(obs.nt)):
        ...
        A = np.hstack([A, np.atleast_2d(np.ones(visit.nsp)).T])
        A = np.vstack(
            [(np.atleast_2d(a).T * np.ones(frames[0].shape)).ravel() for a in A.T]
        ).T
        A = np.hstack(
            [
                A,
        ...
        sigma_w_inv = A[cadence_mask].T.dot(
            A[cadence_mask] / (frames_err[tdx].ravel() ** 2)[cadence_mask, None]
        )
        B = A[cadence_mask].T.dot(
            (frames[tdx] / frames_err[tdx] ** 2).ravel()[cadence_mask]
        )
...
def build_spectrum(obs, errors=False, npoly=3):
    def _make_A(obs):
        ...
        rows = sparse.vstack(
            [sparse.csr_matrix((d * np.ones(obs.shape[:2])).ravel()) for d in diags]
        ).T
        ...
        y, t = (
            sparse.csr_matrix(obs.Y[:, :, 0].ravel() - obs.Y.min()).T,
            sparse.csr_matrix(obs.T[:, :, 0].ravel() - obs.T.min()).T,
        )

        ones = sparse.csr_matrix(np.ones(y.shape[0])).T
        wlc = sparse.csr_matrix(
            (np.atleast_2d(obs.average_lc - 1).T * np.ones(obs.shape[:2])).ravel()
        ).T
        delta_depth = sparse.csr_matrix(
            (
                np.atleast_2d((obs.model_lc != 1).astype(float)).T
                * np.ones(obs.shape[:2])
            ).ravel()
        ).T
    ...
    ws = []
    masks = np.atleast_3d(obs.vsr_mean.mean(axis=0) > 0.93).transpose(
        [2, 0, 1]
    ) * np.ones(obs.shape)
    for idx in tqdm(range(obs.shape[2]), desc="Building Spectrum Model"):
        vsr_trend = sparse.csr_matrix(
            (obs._vsr_grad_model[:, :, idx].T * obs.xshift).T.ravel()
        ).T
        ...
        if errors:
            ...
            y_model_errs[:, :, idx] = (
                A.dot(np.linalg.inv(sigma_w_inv).diagonal() ** 0.5)
            ).reshape(frame.shape)
...
def fit_transit(
    ...
    long_time = [
        (i - i.mean()) / (i.max() - i.min()) for i in np.array_split(x, breaks)
    ]
    orbit_trends = [
        np.hstack(
            [
                (t[idx][o] - t[idx][o].min()) / (t[idx][o].max() - t[idx][o].min())
                - 0.5
                for o in orbits[idx][:, np.any(orbits[idx], axis=0)].T
            ]
        )
        for idx in range(len(t))
    ]

    X = lambda x: np.vstack(
        [
            np.atleast_2d(x[idx]).T * np.diag(np.ones(len(x)))[idx]
    ...
    with pm.Model() as model:
        ...
        r = pm.Normal("r", mu=r, sd=r * 0.3)
        ror = pm.Deterministic("ror", r / r_star_pm)
        b = xo.ImpactParameter("b", ror=ror)

        # Set up a Keplerian orbit for the planets
        orbit = xo.orbits.KeplerianOrbit(
            period=period, t0=t0, r_star=r_star_pm, m_star=m_star, b=b
        )
        ...
        sigma_w_inv = tt.dot(
            A[cadence_mask].T, A[cadence_mask] / yerr[cadence_mask, None] ** 2
        )
        B = tt.dot(
            A[cadence_mask].T,
            (y - (light_curve * normalization))[cadence_mask] / yerr[cadence_mask] ** 2,
        )
        ...
        full_model = pm.Deterministic(
            "full_model", (light_curve * normalization) + noise_model
        )
...
def get_vsr_slant(visit):
    ...
    if hasattr(visit, "model_lc_no_ld"):
        frames_err = frames_err / np.average(
            frames[visit.model_lc_no_ld == 1],
            weights=1 / frames_err[visit.model_lc_no_ld == 1],
            axis=(0),
        )
    ...
    X = np.vstack(
        [visit.X[0][0] ** 0, visit.X[0][0], visit.X[0][0] ** 2, visit.X[0][0] ** 3]
    ).T
    ...
    for tdx in range(visit.nt):
        for idx in range(visit.nsp):
            ...
            B = X[k[tdx][idx]].T.dot(
                frames[tdx][idx][k[tdx][idx]] / frames_err[tdx][idx][k[tdx][idx]] ** 2
            )
            B += prior_mu / prior_sigma ** 2
...
def _get_matrices(visit, npoly=2):
    ...
    arclength = (visit.xshift - np.mean(visit.xshift)) / (
        visit.xshift.max() - visit.xshift.min()
    )
    # arclength -= np.polyval(np.polyfit(visit.time, arclength, 1), visit.time)
    arclength = arclength[:, None, None] * np.ones(visit.shape)

    a = visit.T[:, :, 0].ravel()[:, None]
    b = visit.Y[:, :, 0].ravel()[:, None]
    c = arclength[:, :, 0].ravel()[:, None]
    ...
    if npoly == 0:
        X = np.hstack(
            [
                np.ones(visit.nt * visit.nsp)[:, None],
                a,
                h,
                d,
                d ** 2,
    ...
    if npoly == 2:
        ...
        print(npoly)
    prior_sigma = np.ones(X.shape[1]) * 1e2
    prior_mu = np.zeros(X.shape[1])
    prior_mu[0] = 1
    return X, prior_mu, prior_sigma


def _make_dict(obs, npoly=2, visits="all"):
    ...
    if visits == "all":
        visits = [visit.visit_number for visit in obs]
    ...
    for idx, visit in enumerate(obs):
        ...
        d["Trs"].append(
            (visit.model_lc_no_ld - 1)[:, None, None] * np.ones(visit.shape)
        )
        d["Ecs"].append((visit.eclipse_lc)[:, None, None] * np.ones(visit.shape))
        d["wavelengths"].append(visit.wavelength)
        d["visit_numbers"].append(np.ones(visit.nwav) * idx)

        m = (
            (visit.vsr_mean + visit.vsr_grad)
            * visit.average_lc[:, None, None]
            * visit.spec_mean
        )
        frames = visit.data / m
        frames_err = visit.error / m
        ...
        if visit.nsp > 7:
            ...
            mask = np.ones(visit.Y.shape, bool)
        # mask &= ~bad

        frames_err[~mask] = 1e10
        if visit.filters[0] == "G141":
            frames_err[:, :, visit.wavelength <= 11250] = 1e10
            frames_err[:, :, visit.wavelength >= 16500] = 1e10
        else:
            frames_err[:, :, visit.wavelength <= 7800] = 1e10
            frames_err[:, :, visit.wavelength >= 11300] = 1e10
...
def fit_transmission_spectrum(obs, wav_grid1, wav_grid2, npoly=2, visits="all"):

    td = np.zeros(len(wav_grid1)) * np.nan
    tde = np.zeros(len(wav_grid1)) * np.nan
    ed = np.zeros(len(wav_grid1)) * np.nan
    ede = np.zeros(len(wav_grid1)) * np.nan
    dic = _make_dict(obs, npoly=npoly, visits=visits)
    ...
    for idx, w1, w2 in zip(range(len(wav_grid1)), wav_grid1, wav_grid2):
        ...
        vmasks.append(
            np.array_split(
                vmask, np.where(np.diff(np.hstack(dic["visit_numbers"])) != 0)[0] + 1
            )
        )
        waxis[idx] = np.mean(ws[vmask])

    for wdx in tqdm(np.arange(0, len(wav_grid1)), desc=f"Visit: {visits}"):
        X = []
        prior_sigma = []
        prior_mu = []
        d = []
        ...
        for vdx, vmask in enumerate(vmasks[wdx]):
            ...
            _ = [
                prior_sigma.append(dic["prior_sigma"][vdx]) for i in range(vmask.sum())
            ]
            ...
            ec.append(dic["Ecs"][vdx][:, :, vmask].ravel())
            ti.append(dic["times"][vdx][:, :, vmask].ravel())
        if len(X) == 0:
            continue
        Xl = sparse.lil_matrix(
            (np.sum([len(x) for x in X]), np.sum([x.shape[1] for x in X]))
        )
        lastx = 0
        lasty = 0
        for x in X:
        ...
        Xl = sparse.hstack([Xl, ec[:, None], tr[:, None]], format="csr")
        prior_mu = np.append(prior_mu, [0, 0])
        prior_sigma = np.append(prior_sigma, [1e2, 1e2])

        k = np.ones(len(e), bool)
        for count in [0, 1]:
            sigma_w_inv = Xl[k].T.dot(Xl[k] / e[k, None] ** 2)
            sigma_w_inv += np.diag(1 / prior_sigma ** 2)
            B = Xl[k].T.dot(d[k] / e[k] ** 2)
            B += prior_mu / prior_sigma ** 2
            w = np.linalg.solve(sigma_w_inv, B)
            k = np.abs(d - Xl.dot(w)) / e < 5
    ...
    return (
        ...
        Spectrum(
            waxis,
            ed * obs.eclipse_depth * 1e6,
            ede * obs.eclipse_depth * 1e6,
    ...
    #
    # # Maybe we need a new transmission spectum class...
    # obs.wav_grid = waxis
    # obs.transmission_spec = td * depth * 1e6
    # obs.transmission_spec_err = tde * depth * 1e6
    #
    # obs.emission_spec = ed * obs.eclipse_depth * 1e6
    # obs.emission_spec_err = ede * obs.eclipse_depth * 1e6
