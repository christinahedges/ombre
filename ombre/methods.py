...
def get_flatfield(obs, model):
    ...
    f = f[
        :,
        1014 // 2 - obs.ns // 2 : 1014 // 2 + obs.ns // 2,
        1014 // 2 - obs.ns // 2 : 1014 // 2 + obs.ns // 2,
    ]

    f = np.vstack([f, np.ones((1, f.shape[1], f.shape[2]))])
    X = f[:, obs.spatial.reshape(-1)][:, :, obs.spectral.reshape(-1)][
        :, np.ones((nrow, ncol), bool)
    ].T

    avg = np.nanmean((obs.data / norm / model), axis=0)[np.ones((nrow, ncol), bool)]
    avg_err = ((1 / nt) * np.nansum(((obs.error / norm / model)) ** 2, axis=0) ** 0.5)[
        np.ones((nrow, ncol), bool)
    ]
...
def calibrate(obs, teff, kdx=0):
    ...
    sens /= np.median(sens)
    dw = (
        wav.value[10 + np.argmin(np.gradient(sens[10:-10]))]
        - wav.value[10 + np.argmax(np.gradient(sens[10:-10]))]
    )
    w_mean = np.mean(wav.value)

    def func(params, return_model=False):
        model = np.interp(
            np.arange(0, obs.shape[2]),
    ...
    for idx, a1 in enumerate(a):
        for jdx, b1 in enumerate(b):
            chi[idx, jdx] = np.sum(
                (obs.spec_mean[0, 0] - func([a1, b1], return_model=True)) ** 2
            )
...
def _build_X(
    ...
    for tdx in tqdm(range(obs.nt), desc="Building VSR Component"):
        X1[
            tdx * obs.nsp * obs.nwav : (tdx + 1) * obs.nsp * obs.nwav,
            tdx * As.shape[2] : (tdx + 1) * As.shape[2],
        ] = As[tdx]
    ...
    if spectrum:
        ...
        def _make_A(obs, npoly):
            ...
            y, t = (
                sparse.csr_matrix(obs.Y[:, :, 0].ravel()).T,
                sparse.csr_matrix(obs.T[:, :, 0].ravel()).T,
            )
            ...
            A1 = sparse.hstack(
                [(At2[:, 1:].T.multiply(A)).T for A in Ay.T], format="csr"
            )
    ...
    if bkg:
        ...
        X_bkg = sparse.csr_matrix(
            np.vstack([bkg0.ravel(), bkg1.ravel(), bkg2.ravel(), bkg3.ravel()]).T
        )
    ...
    if spectrum_offset:
        offset = sparse.hstack(
            [
                sparse.csr_matrix((obs.spec_mean == s).ravel().astype(float)).T
    ...
    if transit:
        A = sparse.csr_matrix(
            (
                np.atleast_2d((obs.model_lc - obs.model_lc_no_ld).astype(float)).T
                * np.ones(obs.shape[:2])
            ).ravel()
        ).T
        ...
        A = sparse.csr_matrix(
            (
                np.atleast_2d((obs.model_lc_no_ld - 1).astype(float)).T
                * np.ones(obs.shape[:2])
            ).ravel()
        ).T
        X3 = sparse.lil_matrix((np.product(obs.shape), (A.shape[1] * obs.nwav)))
...
def fit_data(obs):
    ...
    for tdx in range(obs.nt):
        corr[tdx] = np.atleast_2d(
            [
                np.nan
                if d[tdx][idx].mask.all()
                else np.log10(pearsonr(obs.X[tdx][idx][7:-7], d[tdx][idx][7:-7])[1])
                for idx in range(obs.nsp)
            ]
        ).T
    bad_rows |= corr < -3

    edge = (
        np.atleast_3d(obs.vsr_mean.mean(axis=0)).transpose([2, 0, 1])
        * np.ones(obs.shape)
        < 0.98
    )

    model = ma.masked_array(
        m, mask=(obs.error / obs.data > 0.1) | cmr | bad_rows | edge
    )
...
def build_spectrum(obs, errors=False, npoly=2):
    ...
    for idx in tqdm(range(obs.shape[2]), desc="Building Spectrum Model"):
        ...
        if errors:
            y_model_errs[:, :, idx] = (
                A.dot(np.linalg.solve(sigma_w_inv, A.toarray().T)).diagonal() ** 0.5
            ).reshape(frame.shape)
...
def fit_transit(obs, sample=True):
    ...
    o = np.hstack(
        [
            (obs.time[o] - obs.time[o].min()) / (obs.time[o].max() - obs.time[o].min())
            - 0.5
            for o in obs.orbits.T
        ]
    )
    ...
    with pm.Model() as model:
        ...
        t0 = pm.Uniform(
            "t0", lower=t0_val - period / 2, upper=t0_val + period / 2, testval=t0_val
        )
        ...
        r = pm.Normal(
            "r",
            mu=obs.sys.secondaries[0].r.eval(),
            sd=obs.sys.secondaries[0].r.eval() * 0.3,
        )
        ...
        orbit = xo.orbits.KeplerianOrbit(
            period=period, t0=t0, r_star=r_star, m_star=m_star, b=b
        )
        ...
        if np.asarray(obs.forward).all() | np.asarray(~obs.forward).all():
            ...
            noise_model = pm.Deterministic(
                "noise_model", w[0] * obs.xshift + w[1] * o + w[2] * obs.bkg
            )
        else:
        ...
        if np.asarray(obs.forward).all() | np.asarray(~obs.forward).all():
            s = pm.Normal(
                "s", mu=np.asarray([1, 0, 0]), sd=np.asarray([0.1, 0.1, 0.1]), shape=3
            )
            star_model = pm.Deterministic(
                "star_model", s[0] + s[1] * (x - x.mean()) + s[2] * (x - x.mean())
            )
        else:
            s = pm.Normal(
                "s",
                mu=np.asarray([1, 1, 0, 0]),
                sd=np.asarray([0.1, 0.1, 0.1, 0.1]),
                shape=4,
            )
            fw = obs.forward.astype(float)
            bw = (~obs.forward).astype(float)
        ...
        full_model = pm.Deterministic(
            "full_model", (light_curve + noise_model + star_model + hook_model) * norm
        )
